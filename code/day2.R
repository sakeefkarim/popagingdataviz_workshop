...
locations <- tibble(site = c("Bellamy Building (FSU)",
                             "McGill University"),
                    address = c("113 Collegiate Loop, Tallahassee, FL",
                                "McGill University")) %>% 
...
# Here, we mark FSU's Bellamy Building using geom_point()
...
old_age_dependency  <- ggplot(data = select_countries,
                      ...
                      labs(colour = "",
                           x = "", 
                           y = "Old Age Dependency")

old_age_dependency + transition_reveal(year)